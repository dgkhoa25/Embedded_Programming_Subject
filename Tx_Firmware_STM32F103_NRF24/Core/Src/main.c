...
/* USER CODE BEGIN PV */
static Button_Typdef button_btn_1;
static JOYSTICK_state joy_state = OTHER;
static uint32_t JOYSTICK[2];

static uint64_t TxpipeAddrs = 0x11223344AA;
static char myTxData[32];
...
uint8_t JOYSTICK_handle(void)
{
	if ((JOYSTICK[0] >= 2000 && JOYSTICK[0] <= 3000) && (JOYSTICK[1] >= 2000 && JOYSTICK[1] <= 3000))
	{
		...
		return 'S';	// STOP sigal from Tx device
	}
	...
	if (JOYSTICK[0] < 2000 && JOYSTICK[1] >= 2000 && JOYSTICK[1] <= 3000)
	{
		...
		HAL_GPIO_WritePin(xplus_GPIO_Port, xplus_Pin, GPIO_PIN_SET);
		return 'R';	// RIGHT sigal from Tx device
	}
	...
	if (JOYSTICK[0] > 3000 && JOYSTICK[1] >= 2000 && JOYSTICK[1] <= 3000)
	{
		...
		return 'L';	// LEFT sigal from Tx device
	}
	...
	if (JOYSTICK[1] < 2000 && JOYSTICK[0] >= 2000 && JOYSTICK[0] <= 3000)
	{
		...
		HAL_GPIO_WritePin(yplus_GPIO_Port, yplus_Pin, GPIO_PIN_SET);
		return 'B';	// BACKWARD sigal from Tx device
	}
	...
	if (JOYSTICK[1] > 3000 && JOYSTICK[0] >= 2000 && JOYSTICK[0] <= 3000)
	{
		...
		return 'F';	// FORWARD sigal from Tx device
	}
	return 0;
}
// -----------------------------------------------------------------------------------------------
...
void	btn_pressing_callback(Button_Typdef *ButtonX)
{
	...
	switch (joy_state)
	{
		// Controll car by Bluetooth HC05
		case OTHER:
		{
			if (count_key == 0)
			{
		...
		case JOYSTICK_STA:
		{
			if (count_key == 1)
			{
				joy_state = OTHER;
				count_key = 0;
				// Initialize all LED used to indicate the direction for the user at default state
				HAL_GPIO_WritePin(xmin_GPIO_Port, xmin_Pin, GPIO_PIN_RESET);
				HAL_GPIO_WritePin(xplus_GPIO_Port, xplus_Pin, GPIO_PIN_RESET);
				HAL_GPIO_WritePin(ymin_GPIO_Port, ymin_Pin, GPIO_PIN_RESET);
				HAL_GPIO_WritePin(yplus_GPIO_Port, yplus_Pin, GPIO_PIN_RESET);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	// Initialize a Button at PA2 to switch mode between Bluetoot HC05 and NRF24L01
	button_init(&button_btn_1, GPIOA, GPIO_PIN_2);
	
	// Initialize system state at Bluetooth HC05 mode
	joy_state = OTHER;
